...
3. **정책 파일** — `CLAUDE.md`/`AGENTS.md` 등. 컴포넌트·구조·**스킬 매핑** 컨벤션의 SSOT.
...
| **관련 스킬** | 정책 파일의 스킬 매핑을 따름 → 모르면 `find-skills`나 세션 스킬 인벤토리에서 **이름으로** 로드 | **design·build** |
...
> **스킬 이름을 하드코딩하지 마라.** rename·추가·삭제로 썩는다(이 마켓도 `web→dev` 바꾼 적 있음). 관련 스킬은 소비 프로젝트가 켠 번들(`dev`·`vendor@hk-skills` 등)이나 그 repo의 `.claude/skills`에서 온다 — 정책 파일 매핑을 1순위로, 없으면 `find-skills`로 찾고, 아예 없으면 그 단계는 스킬 없이 진행.
...
- **design** — spec sealed 후. 위 의무 SSOT(mockup·스킬·docs·인프라) 대조 후 산출: 컴포넌트 트리 / Props·시그니처 / Mock shape / 서버상태·라우팅 상태전이 / 디자인 토큰 / Edge case + 결정 로그 / Acceptance. **정책 파일의 컴포넌트·구조 컨벤션 100% 반영.** → `sealed` + frontmatter `source_mockup`·`source_skill_refs`·`audit_baseline`.
- **build** — design sealed 후. 코드 편집 직전 스킬·docs 재확인 + Edit 대상 Read. 단위 분리(컴포넌트→화면→page entry), 큰 단위마다 typecheck. **회귀 검증 typecheck/test/build + 시각 확인(dev 서버 + 브라우저)**. pre-commit hook 통과 필수(`--no-verify` 금지). `03-build.md`=작업 로그.
...
| 스킬·docs·mockup 미참조하고 design/build | 구식 패턴·breaking change·토큰 불일치 → 재작성. |
| 스킬 이름 하드코딩(존재 가정) | rename/삭제로 썩음 → 정책 매핑 or `find-skills`. |

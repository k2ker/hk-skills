...
| `hk` | 1 | hk personal cross-project custom commands + hooks + skills + MCP. /hk:pre-clear:save·resume — /clear 전후 세션 컨텍스트 핸드오프. /hk:planning-quick — 페이지/기능 sub-plan 4-phase(spec→design→build→summary) 워크플로우. 스킬 amplify-monitor — AWS Amplify 배포 상태 폴링·검증. 훅: 스킬 확인 넛지·기록 컨벤션 로드·완료 전 기록 게이트. MCP 동봉: context7·figma-desktop. |
